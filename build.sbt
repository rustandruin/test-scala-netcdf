...
runTest <<= (assembly in Compile) map {
  (jarFile: File) => s"spark-submit --driver-memory 4G --class org.apache.spark.mllib.linalg.distributed.netCDFTest ${jarFile} src/main/resources/test.grb2 output.nc" !
}
...
runConvert <<= (assembly in Compile) map {
  (jarFile : File) => s"spark-submit --driver-memory 4G --class org.apache.spark.mllib.linalg.distributed.convertGribToParquet ${jarFile} src/main/resources/filenamelist ${fieldnames} data" !
}
