...
runConvert <<= (assembly in Compile) map {
  (jarFile : File) => s"spark-submit --verbose --master ${System.getenv("SPARKURL")} --num-executors 1  --executor-memory 5G --class org.apache.spark.mllib.linalg.distributed.convertGribToParquet ${jarFile} filenamelist ${fieldnames} data 2>&1 | tee logrun" !
}
